...
def perf_eva(label, pred, title=None, groupnum=None, plot_type=["ks", "roc"], show_plot=True, positive="bad|1", seed=186):
    ...
    dt_sel = var_filter(dat, "creditability")
    ...
    perf_eva(y, dt_pred, show_plot=False)
    
    # Example II # ks & roc plot
    perf_eva(y, dt_pred)
    
    # Example III # ks, lift, roc & pr plot
    perf_eva(y, dt_pred, plot_type = ["ks","lift","roc","pr"])
    '''
...
def perf_psi(score, label=None, title=None, x_limits=None, x_tick_break=50, show_plot=True, seed=186, return_distr_dat=False):
    ...
    train, test = split_df(dt_sel, 'creditability').values()
    ...
    # score ------
    # scorecard
    card = scorecard(bins, lr, X_train.columns)
    # credit score
    ...
    train_score2 = scorecard_ply(train, card, only_total_score=False)
    test_score2 = scorecard_ply(test, card, only_total_score=False)
    # psi
    psi2 = perf_psi(
