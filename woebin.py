...
def woebin(dt, y, x=None, breaks_list=None, special_values=None, 
    ...
    bins_2var = woebin(dat, y = "creditability", 
      x = ["credit.amount", "purpose"])
    
    # Example II
    # binning of the germancredit dataset
    bins_germ = woebin(dat, y = "creditability")
    ...
    bins_cus_brk = woebin(dat_nan, y="creditability",
...
def woebin_plot(bins, x=None, title=None, show_iv=True):
    ...
    # Example II
    bins = woebin(dat, y="creditability")
...
def woebin_adj(dt, y, bins, adj_all_var=True, special_values=None, method="tree"):
    ...
    bins = woebin(dt, y="creditability")
    breaks_adj = woebin_adj(dt, y="creditability", bins=bins)
    bins_final = woebin(dt, y="creditability", breaks_list=breaks_adj)
    
    # Example II
    binsII = woebin(dat, y="creditability")
    breaks_adjII = woebin_adj(dat, "creditability", binsII)
    bins_finalII = woebin(dat, y="creditability", breaks_list=breaks_adjII)
    '''
    # bins concat 
    if isinstance(bins, dict):
        bins = pd.concat(bins, ignore_index=True)
